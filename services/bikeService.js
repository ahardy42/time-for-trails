...
module.exports = async (lat, lng, body, profile) => {
    ...
    const mtbProjectRes = await axios.get(`https://www.mtbproject.com/data/get-trails?lat=${lat}&lon=${lng}&maxDistance=80&maxResults=500&sort=distance&key=${mtbProjectKey}`);
    ...
    const updatedMtbRes = mtbProjectRes.data.trails.filter(el => {
        return utils.updateResponseArray(el, isoChronePolygonArray);
    });

    return {
        trails: updatedMtbRes,
